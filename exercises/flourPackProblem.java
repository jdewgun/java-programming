...
public class flourPackProblem {
    ...
    public static boolean canPack(int bigCount, int smallCount, int goal) {
        // Real Value is 5 times more than the given value
        bigCount *= 5;

        // Decision Flow
        if ((bigCount < 0) || (smallCount < 0) || (goal < 0)) {
            // Retrun false if any value is less than 0.
            return false;
        } else if (smallCount < (goal % 5)) {
            // if Small Count is smaller than the remainder from the goal's division
            // by 5, return false.
            return false;
        }
