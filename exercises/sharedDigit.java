...
public class sharedDigit {
    ...
    public static boolean hasSharedDigit(int integerOne, int integerTwo) {
        boolean digitIsShared = false;

        if ((integerOne >= 10) && (integerOne <= 99) && (integerTwo >= 10) && (integerTwo <= 99)) {
            int integerOneA = integerOne / 10;
            int integerTwoA = integerTwo / 10;
            int integerOneB = integerOne % 10;
            int integerTwoB = integerTwo % 10;

            digitIsShared = ((integerOneA == integerTwoA) || (integerOneB == integerTwoB)
                    || (integerOneB == integerTwoA) || (integerOneA == integerTwoB));

        }

        return digitIsShared;
    }
}