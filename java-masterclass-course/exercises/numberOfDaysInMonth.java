...
public class numberOfDaysInMonth {
    ...
    public static boolean isLeapYear(int year) {
        boolean yearIsALeapYear = false;

        if ((year >= 1) && (year <= 9999)) {
            if (year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)) {
                yearIsALeapYear = true;
            }
        }

        return yearIsALeapYear;
    }

    public static int getDaysInMonth(int monthOfYear, int yearVal) {
        int daysInMonth = -1;

        if ((monthOfYear >= 1) && (monthOfYear <= 12) && (yearVal >= 1) && (yearVal <= 9999)) {
